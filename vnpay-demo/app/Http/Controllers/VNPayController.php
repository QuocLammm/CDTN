...
class VNPayController extends Controller
{
    ...
    {
        // Dữ liệu sản phẩm giả lập (có thể lấy từ database)
        $products = [
            ['id' => 1, 'name' => 'Sản phẩm A', 'price' => 50000, 'image' => 'https://via.placeholder.com/150', 'description' => 'Mô tả sản phẩm A'],
            ['id' => 2, 'name' => 'Sản phẩm B', 'price' => 100000, 'image' => 'https://via.placeholder.com/150', 'description' => 'Mô tả sản phẩm B'],
            ['id' => 3, 'name' => 'Sản phẩm C', 'price' => 150000, 'image' => 'https://via.placeholder.com/150', 'description' => 'Mô tả sản phẩm C'],
        ];

        // Nếu có tham số product_id thì truyền dữ liệu sản phẩm đó
        $selectedProduct = null;
        if ($request->has('product_id')) {
            $selectedProduct = collect($products)->firstWhere('id', $request->product_id);
        }

        return view('test.vnpay_demo', compact('products', 'selectedProduct'));
    }
    ...
    public function createPayment(Request $request)
    {
        ...
        $vnp_Amount = $request->product_price * 100; // Tiền cần thanh toán (VND)
    ...
    {
        return view('test.success');  // Hiển thị trang thành công
    }

    public function paymentFailure()
    {
        return view('test.failure');  // Hiển thị trang thất bại
    }
}
