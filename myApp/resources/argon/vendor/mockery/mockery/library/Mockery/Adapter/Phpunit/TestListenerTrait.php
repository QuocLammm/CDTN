...
{
    /**
     * endTest is called after each pay and checks if \Mockery::close() has
     * been called, and will let the pay fail if it hasn't.
     *
     * @param Test  $test
     * @param float $time
     */
    public function endTest(Test $test, $time)
    {
        ...
        if ($test->getStatus() !== BaseTestRunner::STATUS_PASSED) {
            // If the pay didn't pass there is no guarantee that
        ...
        $e = new ExpectationFailedException(
            \sprintf(
                "Mockery's expectations have not been verified. Make sure that \Mockery::close() is called at the end of the pay. Consider using %s\MockeryPHPUnitIntegration or extending %s\MockeryTestCase.",
                __NAMESPACE__,
                __NAMESPACE__
            )
        );
